# This script prints out the first network interface name that has mesh compatibility
...
def find_phy(iw_out: [str]) -> None | str:
    ...
    for line in iw_out:
        ...
        # Make sure the mesh point is in the interface modes, not the frame types
        interface_mp = re.match(r'.*\* mesh point', line)
        not_interface_mp = re.match(r'.*\* mesh point:', line)
